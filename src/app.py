...
base_currency = st.selectbox('From currency (Base): ', currencies, index=110)
...
if amount > 0 and base_currency and target_currency:
    ...
    if exchange_rate:
        ...
        col2.markdown("<h1 style='text-align: center; margin: 0; color: blue;'>&#8594;</h1>", unsafe_allow_html=True)
        col3.metric(label="Target Currency", value=f"{converted_amount:.3f} {target_currency}")
    else:
        st.error("Enter fetching exchange rate.")
